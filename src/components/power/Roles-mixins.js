export default {
  data() {
    return {
      ...
      // 编辑表单的校验规则element表单校验规则
      editFormRules: {
        // 角色名称校验规则 required//是否必填 message 规则提示 trigger 何事件触发
        roleName: [{ required: true, message: '请输入角色名称', trigger: 'blur' }],
        roleDesc: [{ required: true, message: '请输入角色描述', trigger: 'blur' }]
      },
      // 控制分配权限对话框的显示和隐藏
  ...
  methods: {
    ...
    async showEditDialog(id) {
      ...
      // 把获取到的角色信息，赋值给editForm roleId是后端数据库2定义好的角色id
    ...
    // 点击按钮，更新角色的权限
    async updateRights() {
      // 1.  　getHalfCheckedKeys() 这个方法用来获取父节点半选择状态ID值
      const k1 = this.$refs.tree.getHalfCheckedKeys()
      // 2. getCheckedKeys() 获取到 所有全选状态 的Id值
