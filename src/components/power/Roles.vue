    ...
    <el-card>
      ...
      <el-table :data="roleslist" border stripe style="width: 100%">
        <el-table-column type="expand">
          <!-- 写入三级权限 slot-scope属性 父组件传递了todos数组给子组件 子组件接收遍历把对象传给父组件-->
          <template slot-scope="scope">
            <!-- v-for最外层的el-row scope.row.children children在js中已经定义好-->
            <el-row v-for="(itemOne, indexOne) in scope.row.children" :key="itemOne.id" :class="['borderBottom', indexOne === 0 ? 'borderTop' : '']">
              <el-col :span="4">
                <!-- 点击标签关闭按钮后触发的 close 事件 closable属性可以定义一个标签是否可移除  authName接口参数权限说明-->
                <el-tag closable @close="removeRights(scope.row, itemOne.id)">{{itemOne.authName}}</el-tag>
                <i class="el-icon-caret-right"></i>
              </el-col>
              <el-col :span="20">
                <!-- 在一级权限的后20列中循环生成 二级权限 不断循环上一次循环出来的数据-->
