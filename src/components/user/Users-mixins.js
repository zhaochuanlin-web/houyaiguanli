...
export default {
  data() {
    ...
    return {
      // 查询用户列表时候，要携带的查询参数
      //在接口文档中有参数名1.查询参数2.当前页码3.每页显示的条数
      queryinfo: {
        query: '', // 用户输入的搜索条件
        pagenum: 1, // 当前请求的是第几页数据,在接口文件中有说明
        pagesize: 10 // 每页显示几条数据,在接口文件中有说明
      },
      total: 0, // 总共有多少条数据
      // 用户列表
      userlist: [],
  ...
  //生命周期钩子函数 在渲染成html前调用 初始化某些属性值，再渲染成视图
  created() {
    this.getUserList()
  },
  methods: {
    // 根据查询参数，获取用户列表 异步代码像同步代码ajax 
    //promise是es6解决异步回调 async和await代替了promise 
    //async放在函数前表示该函数是一个异步函数 await关键字只能放到async函数里边 
    //async和await是非阻塞的
    async getUserList() {
      ...
      // 为用户列表赋值 解构赋值 对应users.vue中的prop 动态绑定
    ...
    // 添加新用户
    addUser() {
      // 1. 调用JS方法，校验表单是否符合规范 function(valid)
    ...
    async remove(scope) {
      ...
      //进行异步操作 看接口文档的请求路径和请求方法 200表示成功500表示失败
    ...
    // 点击按钮分配角色 异步函数异步操作看起来像同步操作 200成功 500失败
    async setRole() {
